 ...
 * @LastEditors: 叮咚蛋
 * @LastEditTime: 2020-12-16 10:10:10
 * @FilePath: \hu_sir-contorl\USER\SRC\usart1.c
 */
#include "usart1.h"
...
void USART1_Configuration()
{
	...
	DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)(USART1_DMA_SendBuf);
...
{
	if (usart_dma.Usart1DmaSendFinish == 0) //若上一次传输未完成，则舍弃本次传输
	{
		memcpy(&aTxBuffer_UART1[0], a, count);
		DMA_SetCurrDataCounter(DMA2_Stream7, count);
		DMA_Cmd(DMA2_Stream7, ENABLE);
		usart_dma.Usart1DmaSendFinish = 1;
	}
}
...
void DMA2_Stream7_IRQHandler(void) //数据传输完成，产生中断，检查是否还有没有传输的数据，继续传输
{
	if (DMA_GetITStatus(DMA2_Stream7, DMA_IT_TCIF7) == SET)
	{
		DMA_ClearFlag(DMA2_Stream7, DMA_IT_TCIF7); //清除中断标志
		DMA_ClearITPendingBit(DMA2_Stream7, DMA_IT_TCIF7);
		usart_dma.Usart1DmaSendFinish = 0;
	}
}
