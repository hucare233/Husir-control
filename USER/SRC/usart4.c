 ...
 * @LastEditors: 叮咚蛋
 * @LastEditTime: 2020-12-16 10:10:27
 * @FilePath: \hu_sir-contorl\USER\SRC\usart4.c
 */
#include "usart4.h"
...
void USART4_Configuration()
{
	...
	DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)(UART4_DMA_SendBuf);
...
{
	if (0 == usart_dma.Usart4DmaSendFinish) //若上一次传输未完成，则舍弃本次传输
	{
		memcpy(&aTxBuffer_UART4[0], a, count);
		DMA_SetCurrDataCounter(DMA1_Stream4, count);
		DMA_Cmd(DMA1_Stream4, ENABLE);
		usart_dma.Usart4DmaSendFinish = 1;
	}
}
...
void DMA1_Stream4_IRQHandler(void) //数据传输完成，产生中断，检查是否还有没有传输的数据，继续传输
{
	if (DMA_GetITStatus(DMA1_Stream4, DMA_IT_TCIF4) == SET)
	{
		DMA_ClearFlag(DMA1_Stream4, DMA_IT_TCIF4); //清除中断标志
		DMA_ClearITPendingBit(DMA1_Stream4, DMA_IT_TCIF4);
		usart_dma.Usart4DmaSendFinish = 0;
	}
}
